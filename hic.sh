...
conda activate whdenovo # Do manually if didn't work
\time ls ~/split/split1/ | parallel -j12 "bwa mem -t 6 -R '@RG\tSM:$SAMPLE\tID:$SAMPLE' -B 8 -M $REF ~/split/split1/{} | samtools sort -@4 -O BAM -n > alignment/hic/chunks1/{/.}.nsort.bam" 2> hic.bwa.mem.1.log
\time ls ~/split/split2/ | parallel -j12 "bwa mem -t 6 -R '@RG\tSM:$SAMPLE\tID:$SAMPLE' -B 8 -M $REF ~/split/split2/{} | samtools sort -@4 -O BAM -n > alignment/hic/chunks2/{/.}.nsort.bam" 2> hic.bwa.mem.2.log
...
samtools merge -f -@72 alignment/hic/hic.repaired.bam alignment/hic/repaired/*

samtools fixmate -@ 72 alignment/hic/hic.repaired.bam - | samtools sort -@ 72 - > alignment/hic/hic.fix.sort.bam
